...
* an R function that may be used to preprocess specific R elements before they
  are translated into Prolog.
* a Prolog predicate that may be used to manipulate a term before it is called.
* a Prolog predicate that may be used to manipulate a term after it has been
  called.
...
like `""[1, 2, 3, a, b]` to lists. The argument _expr_ is typically 
...
preprocessing. For `once()` and `submit()`, the function operates on the
bindings. For `findall()`, the function is applied to each alternative using
the `lapply()` iterator.

## Use case 1: empty atoms
...
argument with a list of environment variables. Some of these are empty, though.
Empty atoms that cannot be translated to R symbols because the latter are not
allowed to be empty.
...
## Use case 2: Translating R classes to Prolog dicts

Case study for linear models. 

## Use case 3: Working with named arguments/dicts

* R preprocessing: function arguments to named list
* Prolog: to dict
* Do something with dicts
* Prolog: translate back
* R postprocessing: named list to function arguments
...
A third example in `mathml.Rmd` illustrates rendering of self-written functions
and the use of named function arguments as 
in `integrate(low=0L, up=2L*pi, sin)`.
