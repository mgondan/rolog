...
queries in R computations. The R\ package `rolog` connects to the
SWI-Prolog system, thus enabling deterministic and
...
interface (`is/2`). More complex calculations such as matrix algebra,
...
`rolog` fills this gap through Prolog queries in R scripts,
...
linking to a shared library that embeds the current SWI-Prolog runtime.
...
ingredients to establish communication with the SWI-Prolog runtime. The ways to
...
If a new query _Q_ is created while another query _R_ is still open, a warning
is shown and _R_ is closed.
...
failure, and `clear()`, returning a list collecting the return value of the
...
|`function(x) {sin(x)}`  |lambda(x) :- sin(x)      |function name=attr(name) |
...
\item R matrices are translated to Prolog
  terms `##/R`, `%%/R`, `\$\$\$/R`, and `!!/R` with the
  respective vectors as arguments.
\end{itemize}
%
...
* *realvec* (string): Name of the Prolog term for vectors of
  floats (default is `#`)
* *realmat* (string): Name of the Prolog term for matrices of
  floats (default is `##`)
* *intvec*/*intmat* (string): same for vectors/matrices of 
  integers (default is `%`/`%%`)
* *boolvec*/*boolmat* (string): same for vectors/matrices of 
  logicals (default is `!`/`!!`)
* *charvec*/*charmat* (string): same for vectors/matrices of
  character strings (default is `\$\$`/`\$\$\$`). The single 
  dollar cannot be used because it is the list operator in R.
...
* *preproc* (function with one argument): hook that can be used
  to preprocess R terms before translation (default is dontCheck)
...
```{r}
options(rolog.intvec="%i%")
Q <- call("member", expression(X), list(1:3, 4:6))
once(Q, options=list(intvec="%i%"))
```
...
with `clear()`, or automatically if the query fails. If we are interested in
just the first solution, we can use `once(expr)` as a shortcut to `query(Expr)`,
then `submit()`, then `clear()`. If we want to collect all solutions of a query
with a finite set of solutions, we can use `findall(Expr)`.
...
tools (essentially, RTools under Windows, and xcode under macOS) are properly
...
this gap by providing an interface to a SWI-Prolog distribution in
...
interface, with the need for quoted calls and R expressions for representing
Prolog variables. `rolog` was deliberately chosen to be minimalistic and, so
far, only depends on base R. A more concise representation might be obtained by
tools from the "Tidyverse" ecosystem, as described in Chapter\ 19 of 
