...
author: "Matthias Gondan (Department of Psychology, University of Innsbruck, Austria)
    and Jan Wielemaker (Vrije Universiteit Amsterdam/SWI-Prolog Solutions b.v.)"
date: "2022-11-27"
...
We can see a welcome message from SWI-Prolog. The message can be silenced by the
usual option `quietly=TRUE` of the `library(...)` command.
...
%
\begin{itemize}
\item R vectors of length 0 are translated to Prolog's empty list.
\item R vectors of length 1 are translated to Prolog scalars.
...
* *postproc* (function with one argument): R hook that can be used
  ...
  `postproc(...)` function that reverses the mapping in `preproc(...)`.
...
to `dontCheck` (basically the identity function).
...
the example on mathematical expressions where `r_eval/2` is used to obtain the
name of a function argument, as well as in the next example on interval
arithmetic, where `r_eval/2` is used to evaluate monotonically behaving
R\ functions.
