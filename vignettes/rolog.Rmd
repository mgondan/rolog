...
`query(...)` and `submit()` the query as often as needed or until it fails, as 
is shown below for a query like `member(X, [1, 2, Y])`, which succeeds if _X_ is
...
# no more results (closing the query)
submit()
...
Rolog is a source package that requires the current development version 
of R (i.e. Version 4.2 and above) and the RTools 4.2 toolchain under Windows. 
The reason is a dependence on libarchive [@libarchive] for the installation of 
addons in Prolog, which is not available in RTools 4.0. As soon as R 4.2 is
released, binaries will be made available on https://cran.r-project.org/.
