...
Listing 4.
: R program that uses Rolog to generates MathML from R expressions.

The first example is easy. At the Prolog end, there is a handler for `pbinom/3`
that translates the term into a function-style MathML syntax like
P_bi(X <= k; N, pi), see below.
...
Therefore, mathml.pl includes two handlers that accept terms with named
...
Interfacing the R environment with Prolog widens the range of applications for logic
...
If speed is an issue, some of these steps can, in principle, be moved into the
...
support vectors or matrices, so there is an upper bound in any optimization. Of
course, Prolog's primary purpose is not vector or matrix calculation, so this
