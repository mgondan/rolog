...
returns the name of the function argument of `sin`, that is, `x`.
...
would not print the desired result. A third example in `mathml.Rmd` illustrates
...
number of results and cannot, in general, be vectorized as is often done in R.
...
represent the result of a measurement with limited precision, or a truncated
intermediate result of a student doing hand calculations. A few rules for basic
interval arithmetic are found in `pl/interval.pl`; two calls from R are shown
below.
