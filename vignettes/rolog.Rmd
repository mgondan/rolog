...
stages of the process, using the following hooks:

* an R function `preproc(expr, ...)` that may be used to preprocess specific R
...
* an R function `postproc(expr, ...)` that can be used to postprocess the result
  of the query.
...
The R function `as.rolog(expr, quoted=FALSE)` is meant to simplify this a bit by
...
an _unevaluated_ R call or symbol and then handled using non-standard 
evaluation [@Wickham2019]. This corresponds to Case\ 1 in the snippet below. In
those cases where _expr_ is already quoted, it has to be indicated by the second
argument (Case\ 2 below).

```r
# Case 1
query(member(.X, ""[1, 2, 3, a, b]), preproc=as.rolog)

# Case 2
q <- quote(member(.X, ""[1, 2, 3, a, b]))
query(q, preproc=as.rolog, quoted=TRUE)
```

Note that the name of the variable in the later course will still be _X_, 
