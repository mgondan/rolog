...
See Section\ 4 for one more example with an error resulting from a malformed
query to `r_eval/2`.
...
As show below, R and `rolog` share the same environment.
...
If the R call raises an exception, the error is properly propagated to Prolog
