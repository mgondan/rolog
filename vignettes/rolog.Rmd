...
* an R function that may be used to preprocess specific R elements before they
  are translated into Prolog (see, e.g., the R function `as.rolog()`).
* a Prolog wrapper that manipulates the term before it is called and afterwards
  (see the example with dicts below)
* an R function that can be used to postprocess the result of the query.
...
postproc <- function(x)
{
  # replace Prolog variable by the value of an R variable with the same name
	...
	if(is.call(x))
	  x[-1] <- lapply(x[-1], FUN=postproc)

	if(is.list(x))
	  x <- lapply(x, FUN=postproc)
