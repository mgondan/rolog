...
S <- stringify(R)
unlist(S)   # mainly to save space in the output
@
...
add_node("a"); add_node("b"); add_node("c"); add_node("f"); add_node("u")
add_node("d") # outcome
add_node("e") # exposure

add_arrow("a", "d"); add_arrow("a", "f"); add_arrow("b", "d")
add_arrow("b", "f"); add_arrow("c", "d"); add_arrow("c", "f")
add_arrow("e", "d"); add_arrow("f", "e"); add_arrow("u", "a")
add_arrow("u", "b"); add_arrow("u", "c")
...
MathML can, therefore, be very useful.
...
of rules for translation from R to MathJax is found in \code{pl/mathjax.pl} of 
...
<<echo=TRUE, eval=FALSE, results=tex>>=
...
Therefore, \code{pl/mathjax.pl} includes two handlers. One handler accepts
